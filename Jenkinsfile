...
pipeline{
    ...
    stages {
        ...
        stage('Checkout from Git'){
            steps{
                git branch: 'dev-sec-ops-cicd-pipeline-project-one', url: 'https://github.com/gadeleye/realworld-microservice-project.git'
            }
        }
            ...
            steps{
                script{
                   withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'Docker'){
                       ...
                       sh "docker tag reddit gadeleye/reddit:latest "
                       sh "docker push gadeleye/reddit:latest "
                    }
                }
            }
        }
        stage("Trivy App Image Scan"){
            steps{
                sh "trivy image gadeleye/reddit:latest > trivy_image_analysis_report.txt"
            }
        }
        ...
        stage('ZAP Dynamic Testing | DAST') {
            steps {
                sshagent(['OWASP-Zap-Credential']) {
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@18.224.54.115 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://3.15.153.244:30000/" || true'
    ...
    always {
        ...
        slackSend channel: '#ta-02-devsecops-cicd-alerts', //update and provide your channel name
